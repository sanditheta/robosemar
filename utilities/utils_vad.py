...
class VADFlow:
    def __init__(self,
        ...
        self.min_silence_samples = sampling_rate / 1000 * min_silence_duration_ms 
        self.speech_pad_samples = sampling_rate / 1000 * speech_pad_ms 
        self.reset_states()
    ...
    def __call__(self, x, return_seconds=False):
        ...
        if (speech_prob >= self.threshold) and not self.triggered:
            self.triggered = True
            speech_start = self.current_sample - window_size_samples
            return ('start', int(speech_start) if not return_seconds else round(speech_start / self.sampling_rate, 3))

        if (speech_prob >= self.threshold) and self.triggered:
            speech_start = self.current_sample - window_size_samples
            return ('active', int(speech_start) if not return_seconds else round(speech_start / self.sampling_rate, 3))

        if (speech_prob < self.threshold - 0.15) and self.triggered:
            if not self.temp_end:
                self.temp_end = self.current_sample
            if self.current_sample - self.temp_end < self.min_silence_samples:
                return (False, int(self.current_sample) if not return_seconds else round(self.current_sample / self.sampling_rate, 3))
            else:
                speech_end = self.temp_end - window_size_samples
                self.temp_end = 0
                self.triggered = False
                return ('end', int(speech_end) if not return_seconds else round(speech_end / self.sampling_rate, 3))
...
def utils() -> Tuple:
    utils: Tuple = (get_speech_timestamps,
             save_audio,
             read_audio,
             VADFlow,
             collect_chunks)
